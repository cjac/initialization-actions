...
if [[ "${OS_NAME}" != "ubuntu" && "${CUDA_VERSION}" == "10.1" ]]; then
    unsupported_error
elif [[ "${OS_NAME}" == "rocky" ]]; then
  KERNEL_SUBVERSION=$(uname -r | awk -F- '{print $2}')
  if [[ "${CUDA_VERSION}" == "11.1" && "${KERNEL_SUBVERSION%%.*}" > "305" ]]; then
    unsupported_error
  fi
elif [[ "${OS_NAME}" == "debian" ]]; then
  KERNEL_VERSION=$(uname -r | awk -F- '{print $1}')
  if [[ "${CUDA_VERSION}" < "11.1" && "${KERNEL_VERSION%.*}" > "5.7"  ]]; then
    unsupported_error
  fi
fi
...
function install_nvidia_gpu_driver() {
  if [[ ${OS_NAME} == debian ]]; then
    ...
    if [[ ${NVIDIA_DEBIAN_GPU_DRIVER_VERSION_PREFIX} == 440 ]]; then
      # https://forums.developer.nvidia.com/t/nvidia-440-66-x-cant-build-in-5-6-and-5-7-kernels/140579
      ...
      sh driver.run -x
      cd NVIDIA-Linux-x86_64-440*/
      patch -p1 -i ../kernel-5.6.patch
      patch -p1 -i ../kernel-5.7.patch
      ./nvidia-installer --silent --install-libglvnd
